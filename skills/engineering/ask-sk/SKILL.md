...
- **`/figma-design-agent`** creates or iterates **editable product designs** in Figma from a verified runtime **baseline**. It separates observed facts, approved design decisions, and open questions; inspects existing masters before mutation; reviews one representative state before scaling; and verifies visual fidelity separately from native structure. Reach for it when translating a live application into Figma, extending a product-design system, or carrying Figma work across phase boundaries with a design-state ledger.
